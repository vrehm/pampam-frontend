  ...
  <section class="w-full bg-transparent">
    ...
    <div v-else class="max-w-7xl mx-auto px-4 sm:px-6">
      <div class="flex justify-between items-center md:items-start py-6 md:space-x-10">
        ...
        <nav class="hidden md:flex md:space-x-5 lg:space-x-10">
          <nuxt-link v-for="item in menuItems" :key="item.name" :to="{ path: '/', hash: item.hash }" class="text-base leading-6 font-medium text-gray-900 hover:text-gray-700 transition ease-in-out duration-150">
            {{ item.name }}
          </nuxt-link>
        </nav>
      </div>
