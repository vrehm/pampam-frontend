  ...
  <div class="mx-auto w-auto py-8 bg-yellow-100">
    ...
    <div class="hidden sm:block">
      <nav class="flex justify-center">
        <nuxt-link no-prefetch class="px-3 py-2 font-light font-bold text-sm leading-5 rounded-sm text-gray-700 hover:text-gray-900 focus:outline-none" :to="{ name: 'journal' }">
          Tous
        </nuxt-link>
        <nuxt-link v-for="category in categories" :key="category.id" no-prefetch class="ml-4 px-3 py-2 font-light font-bold text-sm leading-5 rounded-sm text-gray-700 hover:text-gray-900 focus:outline-none" :to="{ name: 'categories-category', params: { category: category.slug } }">
          {{ category.name }}
        </nuxt-link>
      </nav>
    </div>
